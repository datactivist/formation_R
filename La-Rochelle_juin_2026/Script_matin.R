...
glimpse(data)
library(summarytools) #install.packages("summarytools") à éxecuter dans la console si pas déjà installé
view(dfSummary(data))
...
#---- Mise en pratique fin de matinée

# Import des données
place_bleue <- read_csv("https://opendata.agglo-larochelle.fr/d4c/api/records/2.0/downloadfile/format=csv&resource_id=407dd9c7-5f0c-4f69-8746-ba2e1caaa291&use_labels_for_header=true&user_defined_fields=true")
...
place_bleue |> 
    filter(pzb_voie == "Boulevard ANDRE SAUTEL") |> 
    nrow()
